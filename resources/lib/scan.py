...
class Main:
    ...
    def _parse_argv(self):
        ...
        parser.add_argument(
            "theargs", help="the OID of the recording as passed by NPVR", nargs="+")
    ...
    def _init_vars(self):
        ...
        if kodi_source_root:
            self.KODISOURCE = '%s/%s/%s' % (kodi_source_root,
                                            self.TYPE, self.SHOW)
        else:
            self.KODISOURCE = ''
        self.LW.log(['the filepath is ' + self.FILEPATH,
                    'the folderpath is ' + self.FOLDERPATH, 'the type is ' + self.TYPE])
        if USEWEBSOCKETS:
            self.KODIURLS = ['ws://%s:%s/jsponrpc' %
                             (config.Get('kodiuri'), config.Get('kodiwsport'))]
            for remote in config.Get('remotekodilist'):
                self.KODIURLS.append('ws://%s:%s/jsponrpc' % (remote.get('kodiuri'),
                                     remote.get('kodiwsport', default=config.Get('kodiwsport'))))
        else:
            ...
            for remote in config.Get('remotekodilist'):
                self.KODIURLS.append('http://%s:%s@%s:%s/jsonrpc' % (remote.get('kodiuser', default=config.Get('kodiuser')),
                                                                     remote.get(
                                                                         'kodipass', default=config.Get('kodipass')),
                                                                     remote.get(
                                                                         'kodiuri'),
    ...
    def _create_fixes_default(self, default_fix_dir):
        default_fix = '<episodedetails>\r' + \
                      ...
                      '</episodedetails>\r'
        success, loglines = writeFile(default_fix, os.path.join(
            default_fix_dir, 'episode.nfo'), wtype='w')
        self.LW.log(loglines)
    ...
    def _fixes(self):
        self.LW.log(
            ['creating needed files to import recording into Kodi'], 'info')
        found_show = False
        try:
            shows = os.listdir(self.FIXESDIR)
        except FileNotFoundError:
            self.LW.log(
                ['fixes directory does not exist for some reason'], 'error')
            return
        except Exception as e:
            self.LW.log(
                ['unknown error getting list of files in fixes directory', e], 'error')
            return
        self.LW.log(['there are potential shows to process'], 'info')
        self.LW.log(shows)
        if self.SHOW.lower() in map(str.lower, shows):
            self.LW.log(['matched %s with shows to process' %
                        self.SHOW], 'info')
            show_fixdir = os.path.join(self.FIXESDIR, self.SHOW)
            found_show = True
        elif "default" in map(str.lower, shows):
            self.LW.log(['found default template, applying to %s' %
                        self.SHOW], 'info')
    ...
    def _nfofix(self, nfotemplate):
            ...
            if ext in config.Get('thumb_exts'):
                for rename_end in config.Get('rename_ends'):
                    if fileroot.endswith(rename_end):
                        ...
                        item = fileroot[:-len(rename_end)] + \
                            config.Get('thumb_end') + ext
        ...
        self._write_nfofile(nfotemplate, os.path.splitext(
            self.FILEPATH)[0] + '.nfo')
        ...
        for one_file in other_files:
            ...
            if not match:
                success, loglines = deleteFile(
                    os.path.join(self.FOLDERPATH, one_file))
                self.LW.log(loglines)
    ...
    def _special_epnumber(self, items):
        ...
        for item in items:
            ...
            if ext == '.nfo':
                ...
                loglines, results = readFile(
                    os.path.join(self.FOLDERPATH, item))
                self.LW.log(loglines)
                try:
                    season = int(re.findall(
                        '.*<season>(.*)</season>', results)[0])
                    episode = int(re.findall(
                        '.*<episode>(.*)</episode>', results)[0])
                except IndexError:
                    season = None
                    episode = 0
                self.LW.log(['season is %s and episode is %s' %
        ...
        ep_num = str(ep_num)
        self.LW.log(
            ['show had no season or episode, setting season to 0 and epsiode to %s' % ep_num], 'info')
        return ep_num
    ...
    def _generate_thumbnail(self, videopath, thumbpath):
        ...
        if exists:
            if self.SHOW not in config.Get('force_thumbs'):
                self.LW.log(
                    ['thumbnail exists and show is not in force_thumbs, skipping thumbnail generation'], 'info')
                return
            else:
                self.LW.log(
                    ['thumbnail exists but show is in force_thumbs, creating thumbnail'], 'info')
        else:
            self.LW.log(['thumb does not exist, creating thumbnail'], 'info')
        vidcap = cv2.VideoCapture(videopath)
        num_frames = int(vidcap.get(cv2.CAP_PROP_FRAME_COUNT))
        fps = int(vidcap.get(cv2.CAP_PROP_FPS))
        self.LW.log(['got numframes: %s and fps: %s' %
                    (str(num_frames), str(fps))], 'info')
        if num_frames < 30 and fps < 30:
            self.LW.log(
                ['probably an error when reading file with opencv, skipping thumbnail generation'], 'error')
            return
        if config.Get('narrow_time'):
            ...
            frame_start = narrow_start*60*fps + \
                config.Get('begin_pad_time')*60*fps
